...
static void init(struct CircularList* list)
{
	...
	/* allocate memory for the sentinel link */
...
void circularListDestroy(struct CircularList* list)
{
	...
	free(list);
	*/
	while(circularListFront(list) != list->sentinel->value){
...
void circularListRemoveFront(struct CircularList* list)
{
	...
	assert(list != 0);
	assert(list->sentinel->next != list->sentinel);
	struct Link *delete = list->sentinel->next;

	delete->next->prev = list->sentinel;

	list->sentinel->next = delete->next;
