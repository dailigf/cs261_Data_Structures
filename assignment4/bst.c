...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	/* Case 1: val is less than the current node */
	}else{
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	/* create a temporary subTree */
	struct BSTree *subTree = (struct BSTree *)malloc(sizeof(struct BSTree));
	subTree->root = tree->root;


	/* Case 1: node being test is null */
	if(subTree->root == 0){

		contains = 0;

	/* Case 2: value is less than current node */
	}else if(compare(val, subTree->root->val) < 0){
		/*Check to see if a null is reached */
		if(subTree->root == 0){
			contains = 0;

		/* Perform recursive call on the left child */
		}else{
			subTree->root = subTree->root->left;
			contains = containsBSTree(subTree, val);
		}
	/* Case 3: value is greater than the current node */
	}else if(compare(val, subTree->root->val) > 0){
		/* Check to see if a null is reached */
		if(subTree->root == 0){
			contains = 0;
		}else{
			subTree->root = subTree->root->right;
			contains = containsBSTree(subTree, val);
		}

	/* Case 4: if value is found */
	}else{
		contains = 1;
	}
...
TYPE _leftMost(struct Node *cur)
{
	...
	/* Base Case: the current node has no left child */
	if(cur->left == 0){
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if(compare(val, cur->val) == 0){
		...
		/* If there is a right child in the Node to be deleted */
		if(cur->right){
			struct Node *newRoot = _leftMost(cur->right);

			/* need to ensure that the leftmost node of the right child is not the right child itself */
			if(cur->right != _leftMost(cur->right)){
				_removeLeftMost(newRoot);
				free(cur);
				cur = newRoot;
				return cur;
		...
		/* if there is a left child or there is no left child */
		}else{

			struct Node *newRoot = cur->left;
			free(cur);
			cur = newRoot;
			return cur;
...
void testRemoveLeftMost()
{
    ...
    --(tree->cnt);
    printTestResult(tree->root->right == 0, "_removeLeftMost", "removing leftmost of right of root 1st try");
...
void testRemoveNode()
{
    ...
    --(tree->cnt);
    printTestResult(tree->root->val == &(tv->values[0]) && tree->root->left->left == 0, "_removeNode", "remove left of left of root 1st try");
	        
    tree->root = _removeNode(tree->root, &(tv->values[2]));
    --(tree->cnt);
    printTestResult(tree->root->val == &(tv->values[0]) && tree->root->right == 0, "_removeNode", "remove right of root 2st try");
    ...
    --(tree->cnt);
    printTestResult(tree->root == 0, "_removeNode", "remove right of root 4st try");
